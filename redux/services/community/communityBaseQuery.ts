...
export const communityBaseQuery = fetchBaseQuery({
  baseUrl: process.env.NEXT_PUBLIC_COMMUNITY_URL,
  credentials: "include",
...
export const baseQueryWithTokenHandling: BaseQueryFn = async (
  ...
  try {
    const result = await communityBaseQuery(args, api, extraOptions);
