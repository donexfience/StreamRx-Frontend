...
const SourcePanel: React.FC<SourcePanelProps> = ({
  ...
  return (
    <div
      className={`w-96 bg-[#0e1e3c] border-l border-white/10 flex flex-col relative ${
        role === 'guest' ? 'grayscale pointer-events-none' : ''
      }`}
    >
        ...
        {tools.map((tool) => (
          <div key={tool.id} className="mb-5 flex flex-col items-center">
            <button
              ...
              onClick={() =>
                role !== 'guest' && setActiveTool(activeTool === tool.id ? null : tool.id)
              }
            >
              <span>{tool.icon}</span>
            </button>
